...
def get_dataloaders(
    ...
    if cfg.data.data in image_data_name_dict:
    ...
    elif cfg.data.data in ("lm1b", "text8"):
        from scud.data.text import get_text_dataloaders, get_tokenizer, text_data_name_dict

        if cfg.data.data not in text_data_name_dict:
            raise ValueError(f"Unknown text dataset '{cfg.data.data}'.")
        ...
        all_supported = list(image_data_name_dict) + ["lm1b", "text8"] + list(protein_data_name_dict)
